...
typedef struct StrCanaleBass {
	DWORD Canale;  //handle del canale aperto o 0 se vuoto
	short NumeroCd; // numero del suo in corso o -1 se non c'e' niente
	int Loop; // se attivato con loop
}CanaleBassFields;

typedef struct StrBassHandles {
	StrCanaleBass VetCanali[5];
	int CanaleNow; // 0 o 1 usato per sapere quale canale usare.
	DWORD StartOffset;
	float VolumeMusica;
	short OldCdLoop; // ultimo cd su canale 0 e loop
	bool TestPresente;
	HINSTANCE  HandleDll;
	StrProcBassDll Proc;
}BassHandlesFields;
...
typedef struct StrLaraInfo {
	...
	bool TestIsAimingEnemy;  // = true lara is in combat mode and is aiming an enemy. The index of this enemy will be stored in IndexOfAimedEnemy field
	int  IndexOfAimedEnemy;  // index of baddy aimed by lara.
...
typedef struct StrEnumGET {
	...
	int COLOR_RGB;	// return value in GET.Color  INPUT: Index = Id of ColorRgb script command
	int GAME_INFO;	// return values in GET.GameInfo.  INPUT: none
	int SLOT;	// return value in GET.pSlot->  INPUT: index = SLOT_ value
	int INVENTORY;	// return values in GET.pInventory->  INPUT: none
